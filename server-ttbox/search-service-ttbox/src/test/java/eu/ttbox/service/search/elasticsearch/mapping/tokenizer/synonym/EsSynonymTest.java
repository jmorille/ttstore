package eu.ttbox.service.search.elasticsearch.mapping.tokenizer.synonym;
...
public class EsSynonymTest {

	private static Logger log = LoggerFactory.getLogger(EsSynonymTest.class);
	...
	@Test
	@ElasticInitializerSetting(indexName = "synonymidx", indexType = "france",pathSetting = "settings.json",  deleteAfter=false)
//	@ElasticInitializerMapping(pathMapping = "mappings.json")
	...
	public void showAnalysis() {
		for (String searchString : new String[] { "ipod", "universe"}) {
			AnalyzeResponse response = client.admin().indices().prepareAnalyze("synonymidx", searchString).setAnalyzer("synonym_analyzer").execute().actionGet();
