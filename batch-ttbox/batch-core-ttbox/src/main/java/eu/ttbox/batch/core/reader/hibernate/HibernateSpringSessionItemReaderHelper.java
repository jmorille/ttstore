...
public class HibernateSpringSessionItemReaderHelper<T> extends HibernateAccessor  {
	...
	private static final Logger LOG = LoggerFactory.getLogger(HibernateSpringSessionItemReaderHelper.class);
	...
	protected Session getSession() {
		...
		if (LOG.isDebugEnabled()) {
			...
			LOG.debug("Open Hibernate Sessions {}", sessionString );
		}
		// 
		return session;
	}
	...
	public void close() {
		...
		if (statefulSession != null) {
			if (existingTransaction) {
				flushIfNecessary(statefulSession, existingTransaction);
				if (LOG.isDebugEnabled()) {
					String sessionString = org.springframework.orm.hibernate3.SessionFactoryUtils.toString(statefulSession);
					LOG.debug("Close Hibernate Sessions {}", sessionString );
				}
			}
