...
public class SortFileServiceItemWriter implements ItemWriter<File>, InitializingBean {

	private static final Logger log = LoggerFactory.getLogger(SortFileServiceItemWriter.class);
	...
	private String sortFileSuffix = null;
	...
	public List<SortVO> sortFile(File file) throws UnexpectedInputException, ParseException, NonTransientResourceException, Exception {
		...
		if (sortedFileItems != null) {
			...
			// Rename file
			if (sortFileSuffix != null && !sortedFileSuffix.equals(sortFileSuffix)) {
			    sortedFileName = FileUtils.getSuffixedFile(file, sortFileSuffix, compress);
